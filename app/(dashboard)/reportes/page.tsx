...
import {
  ...
  AlertCircle, DollarSign, Users, Zap, BarChart3, Banknote, FileWarning,
} from "lucide-react";
...
export default function ReportesPage() {
  ...
  const tendenciaSemanal = useMemo(() => {
    ...
    return Array.from(map.entries())
      ...
      .map(([semana, vals]) => ({ semana, ...vals, precioM3: vals.m3 > 0 ? vals.ingresos / vals.m3 : 0 }));
  }, [filtered]);
  ...
  const tendencia  = useSemanal ? tendenciaSemanal.map((d) => ({ ...d, dia: d.semana })) : tendenciaDiaria;
  ...
  const porCliente = useMemo(() => {
    ...
    return Array.from(map.entries())
      ...
      .map(([cliente, vals]) => ({
        cliente,
        ...vals,
        precioM3: vals.m3 > 0 ? vals.ingresos / vals.m3 : 0,
        ticketProm: vals.pedidos > 0 ? vals.ingresos / vals.pedidos : 0,
      }));
  }, [filtered]);
  ...
  const porVendedor = useMemo(() => {
    ...
    return Array.from(map.entries())
      .sort(([, a], [, b]) => b.ingresos - a.ingresos)
      .map(([vendedor, vals]) => ({ vendedor, ...vals }));
  }, [filtered]);
  ...
  const porMetodoPago = useMemo(() => {
    ...
    return Array.from(map.entries()).sort(([, a], [, b]) => b.total - a.total)
      .map(([metodo, vals]) => ({ metodo, ...vals }));
  }, [filtered]);
  ...
  const porBomba = useMemo(() => {
    ...
    return Array.from(map.entries()).sort(([, a], [, b]) => b.m3 - a.m3)
  ...
  function exportXLSX() {
    ...
    const detalle = filtered.map((r) => ({
      ...
      "Recibo": r.recibo ?? "", "M³": r.m3Totales ?? "",
      "Precio/m³": r.precioM3 ?? "", "Total": r.total ?? "",
      "Resistencia": r.resistencia ?? "", "Bomba": r.tdBom ?? "",
    ...
    const vendedores = porVendedor.map((v) => ({
      "Vendedor": v.vendedor, "Pedidos": v.pedidos, "M³": +v.m3.toFixed(1), "Total": +v.ingresos.toFixed(2),
    }));
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="bg-[#242424] border border-[#3A3A3A] rounded-xl overflow-hidden">
          <div className="flex flex-col items-center justify-center gap-4 py-28">
            ...
            <p className="text-sm text-gray-400">{loadingLong ? "Cargando información, esto puede tomar unos segundos…" : "Cargando reportes…"}</p>
          </div>
        </div>
      ) : <>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 bg-amber-500/8 border border-amber-500/25 rounded-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-amber-500/70 text-xs ml-2">·  {filtered.length - pagados.length} pedidos sin cobrar en el período</span>
            </div>
          </div>
      ...
      {tab === "ventas" && (
        <div className="space-y-4">
          ...
          {/* Tendencia + Estado de pago */}
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            <div className="lg:col-span-3">
              <Card title={`Total facturado y m³ — tendencia ${useSemanal ? "semanal" : "diaria"}`}>
                <div className="p-5">
                      ...
                      <ResponsiveContainer width="100%" height={150}>
                        <PieChart>
                          <Pie data={porEstado} cx="50%" cy="50%" innerRadius={50} outerRadius={68} paddingAngle={3} dataKey="value">
                            {porEstado.map((e) => <Cell key={e.name} fill={PAGO_COLORS[e.name] ?? "#6B7280"} />)}
                          </Pie>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card title="Ingresos por vendedor">
              <div className="p-5">
                {porVendedor.length === 0 ? <EmptyChart /> : (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={porVendedor} layout="vertical" margin={{ top: 0, right: 8, bottom: 0, left: 8 }}>
                      ...
                      <Bar dataKey="ingresos" fill="#CC2229" radius={[0, 4, 4, 0]} name="Ingresos" />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <Card title="M³ por tipo de entrega (bomba)">
              <div className="p-5">
                {porBomba.length === 0 ? <EmptyChart /> : (
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={porBomba} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                      ...
                      <Bar dataKey="m3" radius={[4, 4, 0, 0]} name="M³">
                        {porBomba.map((_, i) => <Cell key={i} fill={BOMBA_COLORS[i % BOMBA_COLORS.length]} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                )}
          ...
          {/* Por resistencia */}
          <Card title="M³ por resistencia" subtitle="Top 8 resistencias en el período">
            <div className="p-5">
              {porResistencia.length === 0 ? <EmptyChart /> : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={porResistencia} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                    ...
                    <Tooltip contentStyle={TT} formatter={(v, name) => [name === "m3" ? `${Number(v).toLocaleString("es-MX", { maximumFractionDigits: 1 })} m³` : currency(Number(v) || 0), name === "m3" ? "M³" : "Precio/m³"]} />
                    <Legend wrapperStyle={{ fontSize: 11, color: "#9CA3AF" }} />
                    <Bar dataKey="m3" fill="#3B82F6" radius={[4, 4, 0, 0]} name="m3" />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          {porMetodoPago.length > 0 && (
            <Card title="Cobros por método de pago" subtitle="Solo pedidos marcados como pagados">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#2A2A2A]">
                    {porMetodoPago.map((m) => {
                      ...
                      return (
                        <tr key={m.metodo} className="hover:bg-[#2A2A2A] transition-colors">
      ...
      {tab === "operacional" && (
        <div className="space-y-4">
          ...
          {/* Resumen por cliente */}
          <Card title="Ranking de clientes" subtitle="Ordenado por ingresos en el período">
            {porCliente.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#2A2A2A]">
                    {porCliente.map((row, i) => {
                      ...
                      return (
                        <tr key={row.cliente} className="hover:bg-[#2A2A2A] transition-colors">
          ...
          {/* M³ por cliente (chart) */}
          <Card title="M³ por cliente — top 10">
            <div className="p-5">
      ...
      {tab === "cobranza" && (
        <div className="space-y-4">
          ...
          {/* Cartera por cliente */}
          <Card title="Cartera pendiente por cliente" subtitle="Solo clientes con pedidos sin cobrar en el período">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-[#2A2A2A]">
                    {carteraPorCliente.map((row) => {
                      ...
                      const pendPedidos = row.pedidos - row.pagados;
                      return (
                        <tr key={row.cliente} className="hover:bg-[#2A2A2A] transition-colors">
                          <td className="px-4 py-3 text-white font-medium">{row.cliente}</td>
                          <td className="px-4 py-3 text-amber-400 font-semibold">{pendPedidos}</td>
          ...
          <Card title="Pedidos sin cobrar" subtitle="Detalle de todos los pedidos pendientes en el período">
            <div className="overflow-x-auto max-h-[50vh]">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#2A2A2A]">
                  {filtered.filter((r) => !isPagado(r.pagado)).sort((a, b) => (b.total ?? 0) - (a.total ?? 0)).map((r, i) => (
      ...
      {/* ── Detalle de pedidos (siempre visible abajo) ────────────────────── */}
            ...
            <tbody className="divide-y divide-[#2A2A2A]">
              {tableRows.length === 0 ? (
                <tr><td colSpan={10} className="px-4 py-12 text-center text-sm text-gray-600">
                  {filtered.length === 0 ? "Sin pedidos en el período." : "Sin resultados."}
                </td></tr>
              ) : tableRows.map((r, i) => (
                <tr key={r.id ?? i} className="hover:bg-[#2A2A2A] transition-colors">
                  <td className="px-4 py-2.5 text-gray-500 text-xs whitespace-nowrap">
                  ...
                  <td className="px-4 py-2.5 text-gray-200 font-medium text-xs whitespace-nowrap">{r.cliente}</td>
                  <td className="px-4 py-2.5 text-gray-500 text-xs whitespace-nowrap">{r.vendedor || "—"}</td>
                  <td className="px-4 py-2.5 text-[#CC2229] font-mono text-xs">{r.recibo || "—"}</td>
                  ...
                  <td className="px-4 py-2.5 text-gray-400 text-xs">{r.resistencia || "—"}</td>
                  <td className="px-4 py-2.5 text-gray-400 text-xs">{r.tdBom || "—"}</td>
