...
import {
  ...
  Clock, Download, Pencil, Plus, Trash2, X,
} from "lucide-react";
...
interface FormState {
  ...
  hora: string; chofer: string; cr: string; horaSalida: string;
  remision: string; numSello: string;
  horaLlegadaObra: string; horaInicioDescarga: string; horaFinalDescarga: string;
  horaSalidaObra: string; tiempoDescarga: string; hsr: string; tiempoExtraDescarga: string;
  m3Totales: string; m3: string; extras: string; tdBom: string;
  resistencia: string; color: string; m3Vacios: string;
  precioM3: string; precioM3Bomba: string; ltoAcelr: string; kiloFibra: string;
  m3Imper: string; aditivo: string; tuberiaExtra: string; permisosOC: string;
  totalXM3: string; total: string;
...
function calcTiempoDescarga(inicio: string, fin: string): string {
  ...
  const diffMin = (fh * 60 + fm) - (ih * 60 + im);
  if (diffMin <= 0) return "";
...
function emptyForm(dia: string): FormState {
  return {
    ...
    hora: "", chofer: "", cr: "", horaSalida: "", remision: "", numSello: "",
    horaLlegadaObra: "", horaInicioDescarga: "", horaFinalDescarga: "",
    horaSalidaObra: "", tiempoDescarga: "", hsr: "", tiempoExtraDescarga: "",
    m3Totales: "", m3: "", extras: "", tdBom: "", resistencia: "", color: "", m3Vacios: "",
    precioM3: "", precioM3Bomba: "", ltoAcelr: "", kiloFibra: "", m3Imper: "",
    aditivo: "", tuberiaExtra: "", permisosOC: "", totalXM3: "", total: "",
...
function formFromProg(p: Programacion): FormState {
  return {
    ...
    hora: p.hora, chofer: p.chofer, cr: p.cr, horaSalida: p.horaSalida,
    remision: p.remision, numSello: p.numSello,
    horaLlegadaObra: p.horaLlegadaObra, horaInicioDescarga: p.horaInicioDescarga,
    horaFinalDescarga: p.horaFinalDescarga, horaSalidaObra: p.horaSalidaObra,
    tiempoDescarga: p.tiempoDescarga, hsr: p.hsr, tiempoExtraDescarga: p.tiempoExtraDescarga,
    m3Totales: p.m3Totales != null ? String(p.m3Totales) : "",
    m3: p.m3 != null ? String(p.m3) : "",
    ...
    aditivo: p.aditivo, tuberiaExtra: p.tuberiaExtra != null ? String(p.tuberiaExtra) : "",
    permisosOC: p.permisosOC,
    totalXM3: p.totalXM3 != null ? String(p.totalXM3) : "",
...
function exportCSV(rows: Programacion[]) {
  const headers = [
    "DÍA","VENDEDOR","DÍA Y HORA PEDIDO","MUESTRAS","HORA","CHOFER","CR",
    "HORA SALIDA","REMISIÓN","NUM SELLO","HORA LLEGADA OBRA","HORA INICIO DESC.",
    "HORA FINAL DESC.","HORA SALIDA OBRA","TIEMPO DESC.","HSR","TIEMPO EXTRA DESC.",
    "CLIENTE","TELÉFONO","M3 TOTALES","M3","EXTRAS","T/D BOM","RESISTENCIA",
    "PARA USO","DIRECCIÓN","PRECIO M3","$ M3 BOMBA","COLOR","T.EXTRA DESC.",
    "M3 VACÍOS","$ LTO ACELR","$ KILO FIBRA","$ M3 IMPER","ADITIVO",
    "TUBERÍA EXTRA","PERMISOS O/C","TOTAL X M3","TOTAL","RECIBO",
    "CRÉDITO","FACT","PAGADO","MÉTODO PAGO","FECHA PAGO",
  ];
  const lines = rows.map((r) => [
    r.dia, r.vendedor, r.diaHoraPedido, r.muestras, r.hora, r.chofer, r.cr,
    r.horaSalida, r.remision, r.numSello, r.horaLlegadaObra, r.horaInicioDescarga,
    r.horaFinalDescarga, r.horaSalidaObra, r.tiempoDescarga, r.hsr, r.tiempoExtraDescarga,
    r.cliente, r.telefono, r.m3Totales ?? "", r.m3 ?? "", r.extras, r.tdBom, r.resistencia,
    r.paraUso, r.direccion, r.precioM3 ?? "", r.precioM3Bomba ?? "", r.color,
    r.tiempoExtraDescarga, r.m3Vacios ?? "", r.ltoAcelr ?? "", r.kiloFibra ?? "",
    r.m3Imper ?? "", r.aditivo, r.tuberiaExtra ?? "", r.permisosOC,
    r.totalXM3 ?? "", r.total ?? "", r.recibo, r.credito, r.fact, r.pagado,
    r.metodoPago, r.fechaPago,
  ].map((v) => `"${v}"`).join(","));
...
function FormDrawer({
  ...
  const tiempoAuto = calcTiempoDescarga(form.horaInicioDescarga, form.horaFinalDescarga);
  ...
  async function handleSave() {
    ...
    try {
      ...
      await onSave({
        ...
        numSello: form.numSello.trim(),
        horaLlegadaObra: form.horaLlegadaObra,
        horaInicioDescarga: form.horaInicioDescarga,
        horaFinalDescarga: form.horaFinalDescarga,
        horaSalidaObra: form.horaSalidaObra,
        tiempoDescarga: tiempoAuto,
        hsr: form.hsr,
        tiempoExtraDescarga: form.tiempoExtraDescarga.trim(),
        m3Totales: n(form.m3Totales),
        m3: n(form.m3),
        ...
        permisosOC: form.permisosOC.trim(),
        totalXM3: n(form.totalXM3),
        total: n(form.total),
    ...
    <div className="fixed inset-0 z-[100] flex">
      ...
      <div className="relative ml-auto flex h-full w-full max-w-2xl flex-col bg-white border-l border-gray-200 shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <Sec title="Programación del viaje" />
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              ...
              <select value={form.chofer} onChange={(e) => set("chofer", e.target.value)} className={inp}>
            ...
            <div>
              ...
              <input type="text" value={form.cr} onChange={(e) => set("cr", e.target.value)} placeholder="CR-01" className={inp} />
            </div>
            <div>
              <label className={lbl}>Hora de salida</label>
              <input type="time" value={form.horaSalida} onChange={(e) => set("horaSalida", e.target.value)} className={inp} />
            </div>
            <div>
              <label className={lbl}>Remisión</label>
              <input type="text" value={form.remision} onChange={(e) => set("remision", e.target.value)} placeholder="18945" className={inp} />
            </div>
            <div>
              <label className={lbl}>Num. de sello</label>
              <input type="text" value={form.numSello} onChange={(e) => set("numSello", e.target.value)} placeholder="—" className={inp} />
            </div>
          </div>

          <Sec title="Tiempos en obra" />
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>Hora llegada a obra</label>
              <input type="time" value={form.horaLlegadaObra} onChange={(e) => set("horaLlegadaObra", e.target.value)} className={inp} />
            </div>
            <div>
            ...
            <div>
              <label className={lbl}>Hora inicio de descarga</label>
              <input type="time" value={form.horaInicioDescarga} onChange={(e) => set("horaInicioDescarga", e.target.value)} className={inp} />
            </div>
            <div>
              <label className={lbl}>Hora final de descarga</label>
              <input type="time" value={form.horaFinalDescarga} onChange={(e) => set("horaFinalDescarga", e.target.value)} className={inp} />
            </div>
            <div>
              <label className={lbl}>Hora salida de obra</label>
              <input type="time" value={form.horaSalidaObra} onChange={(e) => set("horaSalidaObra", e.target.value)} className={inp} />
            </div>
            <div>
              <label className={lbl}>Tiempo de descarga</label>
              <input
                type="text"
                value={tiempoAuto || "—"}
                readOnly
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-emerald-700 font-mono font-semibold cursor-default select-none"
              />
            </div>
            <div>
              <label className={lbl}>Tiempo extra de descarga</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>M3 totales</label>
              <input type="number" step="0.5" min="0" value={form.m3Totales} onChange={(e) => set("m3Totales", e.target.value)} placeholder="0.0" className={inp} />
            </div>
            <div>
              <label className={lbl}>M3</label>
              <input type="number" step="0.5" min="0" value={form.m3} onChange={(e) => set("m3", e.target.value)} placeholder="0.0" className={inp} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={form.permisosOC} onChange={(e) => set("permisosOC", e.target.value)} placeholder="—" className={inp} />
            </div>
            <div>
              <label className={lbl}>Total x M3 $</label>
              <input type="number" step="0.01" min="0" value={form.totalXM3} onChange={(e) => set("totalXM3", e.target.value)} placeholder="0.00" className={inp} />
            </div>
            <div>
              <label className={lbl}>Total $</label>
              <input type="number" step="0.01" min="0" value={form.total} onChange={(e) => set("total", e.target.value)} placeholder="0.00" className={inp} />
            </div>
          </div>
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-4 py-3 text-gray-200 text-sm whitespace-nowrap">{p.chofer || <span className="text-gray-600">—</span>}</td>
        <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{p.cr || "—"}</td>
        ...
        <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{p.remision || "—"}</td>
        <td className="px-4 py-3 whitespace-nowrap">
          {p.resistencia
        ...
        <tr className="bg-[#0F1115]">
          <td colSpan={10} className="px-6 py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 text-xs">
              {[
                { label: "Hora salida", value: p.horaSalida },
                { label: "Hora llegada obra", value: p.horaLlegadaObra },
                { label: "Inicio descarga", value: p.horaInicioDescarga },
                { label: "Final descarga", value: p.horaFinalDescarga },
                { label: "Hora salida obra", value: p.horaSalidaObra },
                { label: "Tiempo descarga", value: p.tiempoDescarga },
                { label: "T. extra descarga", value: p.tiempoExtraDescarga },
                { label: "HSR", value: p.hsr },
                { label: "Num. sello", value: p.numSello },
                { label: "M3", value: p.m3 != null ? `${p.m3} m³` : "" },
                ...
                { label: "Permisos O/C", value: p.permisosOC },
...
export default function ProgramacionPage() {
  ...
  function openEdit(p: Programacion) {
    setEditing(p);
    setShowDrawer(true);
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#2A2A2A]">
              {loading ? (
                <tr>
                  <td colSpan={10} className="px-4 py-12 text-center text-sm text-gray-600">Cargando…</td>
                </tr>
              ) : delDia.length === 0 ? (
                <tr>
